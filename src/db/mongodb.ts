...
async function ensureIndexes(db: Db): Promise<void> {
  ...
  await Promise.all([
    ...
    db.collection("missionTemplates").createIndex({ templateId: 1 }, { unique: true }),
    db.collection("missions").createIndex({ missionId: 1 }, { unique: true }),
    db.collection("missions").createIndex({ templateId: 1, updatedAt: -1 }),
    db.collection("missionAttempts").createIndex({ attemptId: 1 }, { unique: true }),
    db.collection("missionAttempts").createIndex({ learnerId: 1, updatedAt: -1 }),
    db.collection("missionAttempts").createIndex({ learnerId: 1, missionId: 1, status: 1 }),
    db.collection("missionSteps").createIndex({ missionId: 1, stepId: 1 }, { unique: true }),
    db.collection("learnerMissionProgress").createIndex({ learnerId: 1, missionId: 1 }, { unique: true }),
    db.collection("learnerMissionProgress").createIndex({ learnerId: 1, updatedAt: -1 }),
    db.collection("rewardClaims").createIndex({ claimId: 1 }, { unique: true }),
    db.collection("rewardClaims").createIndex({ learnerId: 1, createdAt: -1 }),
    db.collection("forestStates").createIndex({ learnerId: 1 }, { unique: true }),
    db.collection("forestEvents").createIndex({ eventId: 1 }, { unique: true }),
    db.collection("forestEvents").createIndex({ learnerId: 1, createdAt: -1 }),
