...
import { buildChildSummary, recentActivity } from "@/services/parentInsights";
...
export async function GET(_req: Request, { params }: { params: { childId: string } }) {
  ...
  const [summary, activity] = await Promise.all([buildChildSummary(learner), recentActivity(learner.learnerId, learner.sessionId)]);
  return NextResponse.json({ success: true, data: { summary, activity } });
}
